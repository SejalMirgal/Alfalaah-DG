            ...
            if (!FilterWord(txtMessage.Text))
            {
                if (Session["captcha"] != null && Session["captcha"].ToString() == txtCaptcha.Text)
                {
                    ...
                    else
                    {
                        ...
                        string Body = String.Format("<table border='1' style='border-collapse: collapse;border: 1px solid black;'><tr><td>Name</td><td>{0}</td></tr><tr><td>Phone</td><td>{1}</td></tr><tr><td>Email</td><td>{2}</td></tr><tr><td>Nature</td><td>{3}</td></tr><tr><td>Message</td><td>{4}</td></tr><tr><td>IP</td><td>{5}</td></tr></table>", txtFirstName.Text, txtMobile.Text, txtEmail.Text, ddlnature.SelectedValue, txtMessage.Text, GetIpAddress());
